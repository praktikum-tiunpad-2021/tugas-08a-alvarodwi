...
template <typename T> using ElementPtr = Element<T> *;
 ...
 * @return  queue kosong
 */
template <typename T> Queue<T> new_queue() {
  Queue<T> q;
  q.head = nullptr;
  q.tail = nullptr;
  return q;
 ...
 */
template <typename T> void enqueue(Queue<T> &q, const T &value, int priority) {
 ...
 */
template <typename T> T top(const Queue<T> &q) { return q.head->data; }
 ...
 * @param q   queue yang dipakai.
 */
template <typename T> void dequeue(Queue<T> &q) {
  ElementPtr<T> target;
  if (q.head == nullptr && q.tail == nullptr) {
    target = nullptr;
